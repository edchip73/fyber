 ...
 * Relatório de contas
 */
class VendaReport extends Page
{
    ...
    public function __construct()
    {
        ...
        $this->form = new FormWrapper(new Form('form_relat_venda'));
        ...
        $this->form->addField('Pagamento Inicial', $data_ini, '50%');
        $this->form->addField('Pagamento Final', $data_fim, '50%');
    ...
    public function onGera()
    {
        ...
        try
        {
            ...
            $repositorio = new Repository('View_Venda');

            // cria um critério de seleção por intervalo de datas
            $criterio = new Criteria;
            $criterio->setProperty('order', 'Pagamento');
            
            if ($dados->data_ini)
                $criterio->add('Pagamento', '>=', $data_ini);
            if ($dados->data_fim)
                $criterio->add('Pagamento', '<=', $data_fim);
    ...
    public function onGeraPDF($param)
    {
        ...
        $filename = 'tmp/Relatório_Vendas.pdf';
        if (is_writable('tmp'))
        {
